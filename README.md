...
- Friends can also be added by invite (Stats → Friends): "My invite" shows a QR code
  and a shareable `streetpass://` link signed with your key (it carries your ID and
  nickname, nothing else); "Add friend" scans a QR code or accepts a pasted link. A
  link shared from a messenger can be sent to StreetPass via "Share". Camera access is asked for only on
  the scanner screen.
